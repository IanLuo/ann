...
```
 spawn → materialize → [GATE① grilling] → validate → activate → execute → verify → [GATE② confirm] → commit
```
...
- **activate:** frontmost-ready selection — lowest prefix among ready siblings; failed/superseded siblings skipped.
...
- A round completes only when all its tasks' goals are met + the round root's ACs verified → `completed` event → next round may spawn.
