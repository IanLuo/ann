...
| the journey format contract (engine data contract): table of legs (epics, sequential gates), parallel task groups, immutable `node.json`, append-only `events.jsonl` (tasks only), derived leg status, searchable `description.md`, per-node artifacts, depth policy, checkpoint/RPO, read discipline | `journey/legs/06-engine-build/03-format-amendment-v8/artifacts/journey-format-spec-v8.md` (locked @ 6fe3c1e) |
| flow control: lifecycle, human gates, rejection/rework, resolution ladder, per-work-type flows, closure task | `journey/legs/06-engine-build/02-flow-control-v4/artifacts/flow-control-spec-v4.md` (locked @ 43beff2) |
