...
| the journey format contract (engine data contract): table of legs (epics, sequential gates), parallel task groups, immutable `node.json`, append-only `events.jsonl` (tasks only), derived leg status, searchable `description.md`, per-node artifacts, depth policy, checkpoint/RPO, read discipline; **v9: the artifact model — ref (link into docs/) + commit clues + conclusion; every task concludes with an artifact** | `journey/legs/06-engine-build/13-format-amendment-v9/artifacts/journey-format-spec.md` (locked @ 3c04f99) |
| flow control: lifecycle, human gates, rejection/rework, resolution ladder, per-work-type flows, closure task; **v5: artifact completion rule (6/7 contradiction resolved)** | `journey/legs/06-engine-build/13-format-amendment-v9/artifacts/flow-control-spec.md` (locked @ 3865cfb) |
