...
| the journey format contract (engine data contract): table of legs (epics, sequential gates), parallel task groups, immutable `node.json`, append-only `events.jsonl` (tasks only), derived leg status, searchable `description.md`, per-node artifacts, depth policy, checkpoint/RPO, read discipline | `journey/legs/06-engine-build/00/01-format-amendment-v7/artifacts/journey-format-spec-v7.md` (locked @ bee8e89) |
| flow control: lifecycle, human gates, rejection/rework, resolution ladder, per-work-type flows, closure task | `journey/legs/06-engine-build/00/02-flow-control-v4/artifacts/flow-control-spec-v4.md` (locked @ 43beff2) |
