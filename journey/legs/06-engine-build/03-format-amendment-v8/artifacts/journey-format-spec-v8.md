...
# Journey Format Spec (v8)
*Artifact of task `06-engine-build/03-format-amendment-v8`. Type: spec. Complete superseding version — v7 (locked @ bee8e89) + the flat task group (tasks live directly under the leg — no level dirs; the 00/ segment removed; sanctioned migration of L4–L6). Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v7 (locked @ bee8e89). Referrers: flow-control-spec · change-protocol · ann-system-design · architecture · AGENTS.md · implementation slices.*
