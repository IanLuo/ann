...
| `present!` | `<id> grill|confirm [confirmedSha]` | WRITE — present a task's work at a gate for the HUMAN's decision: records `submitted` and the task waits (blocked) for the human's `gate! accept|reject`; an interrupted gate stays blocked (resumable), never looks un-started. `[confirmedSha]` (confirm gate only) binds the decision to the exact bytes under review |
