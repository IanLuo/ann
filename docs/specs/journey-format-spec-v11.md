<!-- specs:locked:4b4c8c3 2026-08-22 type=spec -->
...
# Journey Format Spec (v11)
*Artifact of task `06-engine-build/15-format-amendment-v11`. Type: spec. Complete superseding version — v10 (locked @ 51c7764) + the **TASK CONTRACT CHECKLIST** (§2): a task is self-sufficient when a fresh agent reads only its `node.json` contract + resolvable inputs and can execute without guessing — intent · ACs · grounded inputs (logical names resolving via `current()`) · target areas · expected outputs · open questions · determinable work type. **F-AC19** (§7) enforces items 1–3 at spawn (reject) and in `check()` (v9+ nodes). Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v10 (locked @ 51c7764). Referrers: flow-control-spec · change-protocol · ann-system-design · architecture · AGENTS.md · implementation slices.*
...
## 15. docs/ layout & the migrations (v9 — NEW; v10 amended)

**Where shared content lives.** `docs/` holds **only the SHARED contract stack** — docs cross-referenced by logical name across the journey — categorized by artifact type, one place per file; git tracks changes; the tree holds refs (symlinks) + events. **No catch-all category.** Everything else is task-local and lives in the task's `artifacts/` (real files).
