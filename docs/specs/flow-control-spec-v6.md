<!-- specs:locked:5898f89 2026-08-22 type=spec -->
...
# Flow Control Spec (v6)
*Artifact of task `06-engine-build/14-format-amendment-v10`. Type: spec. Complete superseding version — v5 (locked @ 3865cfb) + the implementation artifact becomes STRUCTURED COMMIT EVIDENCE in events (journey-format-spec v10 §3/§14) — the record doc is deprecated; the §6/§7 completion rule aligns. Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v10 (locked). Referrers: S5 planner kernel, implementation slices, review-task.*
...
- Immutable once locked. Amendments = **new nodes** whose artifacts **completely supersede** this spec (back-reference + `superseded` event). v5 resolved the v4 §6/§7 contradiction via the record doc; v6 aligns with journey-format-spec v10 — the implementation artifact is **structured commit evidence in events** (machine-readable, never prose-parsed), and the record doc is deprecated (task 05's `s2-record` grandfathered).
