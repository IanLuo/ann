...
| **schema (v3 — NEW)** | the store's vocabulary — ONE instance: `rules/schema/vocab.json` | `eventTypes` · `statuses` (**v5: `rework` included**) · `gates` · **`gateDecisions` (v4, leg 12/08)** · `artifactTypes`. **Data-or-code is per-key and is stated in §5:** `artifactTypes` is ADJUSTABLE data (each entry carrying its category + versioned flag, §9); `eventTypes` / `statuses` / the gate SET and POSITIONS are **PROTECTED** — enforced as code literals, recorded as reconciliations |
...
| **`vocab.statuses` — `rework`** (v5 — leg 12/08 rework) | the `workflowState` projection (`src/store/workflow.ts`): the word for a task whose bound gate's LAST decision is a rejection with no later submission at it (a re-submission clears it), bounded by the `REWORK_EXEMPT` set (= `CLOSED_TASK_STATUSES` + `failed` — a knowing duplicate of `NOT_OPEN_TO_RUN`, pinned by a test); plus `READY_STATUSES`, which deliberately EXCLUDES it, and the derived `next` verdict `{verdict: 'rework', gate}` | status is a DERIVATION over the event tail — the list documents the function's range; `rework` must not be a registry-only word, or a rejected-gate task falls back to the `queued` default and looks like fresh work to `frontmostReady` / `advance()` / the driver — the exact mis-dispatch that drove 12/05 twice | adding `rework` to `vocab.json` alone derives nothing: the projection, the exempt set and `READY_STATUSES` must change with it (a CODE change). The word is NOT in `CLOSED_TASK_STATUSES` (the leg aggregate, the leg gate and the `distance-to-goal` rule still count the task as OPEN work) and NOT in `READY_STATUSES` (nothing proposes it) |
