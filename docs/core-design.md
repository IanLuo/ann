# Ann Core Design — v14 (for lock)

*Re-implementation target. The locked substrate (journey-format v13, store single-writer, packet spec, provider adapter, S4 validators, honesty-layer engines) stays. Incorporated: design reviews #1–#13 (2026-08-25/26) + owner decisions (2026-08-26: general config; conditional chains; verify fail cycles). The design target: a system that is SIMPLE and FLEXIBLE — the machinery is small and stable; everything that varies (flows, steps, chains, gate sources, rework) is ADJUSTABLE DATA, so later flow changes are sound changes, not design changes.*
...
**THE GENERAL CONFIG (owner decision, 2026-08-26):** a user-facing config file, `rules/config/default.json` (per-project registry data — the resource-registry pattern; the personal overlay stays at `~/.ann/config.json` for provider/model/credentials). **Precedence per leaf key: `env > user > project > builtin`** (resource-registry-v2:98). It holds the END-USER KNOBS: `flow.conditionals` (lifts the linear-chain limit — PROJECT SEMANTICS, **NOT user-overridable**) and `flow.verifyFailCycles` (bounded retry at the verify rung — a cost knob, user-overridable: the overlay "overrides preferences **and `flow.verifyFailCycles`**"), plus F17's spec'd preferences (ask-vs-assume, defaults). **(resource-registry v5 — the SERVE BIND joins the class, leg 10 task 03):** the config ALSO carries `server.host` + `server.port` — where `ann serve` listens (builtin `127.0.0.1:8787`; env layer `ANN_HOST`/`ANN_PORT`; project registry + user overlay overridable, since which address my own machine's server listens on is a MACHINE preference, not journey semantics; per invocation `--host`/`--port` outrank every layer). The GATE REJECT BOUND (3) and the gate set/positions stay LOCKED constants — the config moves the limits the design chose, not the invariants the store enforces. Full section: §1/§6/§8.
...
| F-AC16 closure · F-AC18 conclusion + traceability | **post-hoc `check()`** | check-time validations, not write-path invariants — stated. **(v18 — the ONE exception: the STRUCTURED conclusion is ALSO an L1 write-path refusal — `complete!` refuses, named and writing nothing, an unclaimed contract AC or a conclusion with no `result:'pass'` check bound to a cited commit; journey-format-spec v18 §3/§7)** |
...
| **verify-fail cycles** | **configurable** (v12-v14) | `flow.verifyFailCycles: N` — default 1, CEILING 3 (a constant; NFR-CST-1 — never open). Anchor: **since the latest gate decision at `confirm`, resetting per rework pass**. Ceiling consequence: **≤ 3 × 3 = ≤ 9 chain executions** (the knob multiplies inside every rework pass — "the verify rung only" is qualified). Cycles advance the attempt boundary for execute/confirm steps only (§2); recorded as `kind:'verify'` `{cycle}`. Empty chain → inert (forced 1) + `waiting` event → blocked. The GATE reject bound stays a locked constant |
| **the general config registry** (`rules/config/default.json`) | **data, adjustable** | the user-facing knobs (conditionals · verify cycles · ask-vs-assume · defaults · **(v5)** the serve bind `server.host`/`server.port`); **precedence `env > user > project > builtin`, per leaf key — `flow.conditionals` is NOT user-overridable** (project semantics; the product IS the journey — review-13 F), while (v5) the `server.*` bind IS (a machine preference); read via the resource-registry pattern — never hardcoded |
...
**The coordination loop:** get store info (L1 reads) → work the current task → run the step interface → translate intents → emit via L1 → observe gates → repeat. **Resumable** — interruption (pending gate, rejection, crash, verify-wait) leaves the task blocked/pending; the resume point is **DERIVED from the event tail**, in FOUR tail states: *last-at-gate `confirmed` → skip the gate WRITE (the step still re-runs from the transcript — a decided gate skips the write, never the step); `submitted` undecided → block and wait (produced by `submit!`); last `rejected` → the LOCKED rework rung runs and the gate IS re-obtained (flow-control v6:36-38); a `waiting` record with no subsequent commit evidence → block and wait (the empty-chain verify-wait — review-13 E)*. **`execute` is NEVER skipped**; its replay safety = **intent idempotence + the transcript**, discriminated by **TRANSCRIPT PRESENCE** (§2 — NEVER `ctx.feedback`, which review-8 measured broken). **L2 is not a monolithic loop** — gates are L1 writes the flow observes.
...
- **The TRACE is a TRANSCRIPT** (review-7 N2, review-8 measured): `llm`/`interact` emit runtime records — keyed `(taskId, stepId, runId, seq)` with the LLM completion text and the interact question+answer — through an L2-owned record hook ending at `append!`, in a **schema-legal home**: the v14 §3 amendment lets an `evidence` event carry an optional structured `trace` record `{stepId, runId, seq, kind: 'llm'|'ask'|'research'|'decide', prompt?, completion?, question?, answer?, research?: [{topic, findings, sources?[]}], options?[]}` — shape-policed for its FOUR STEP-LEVEL kinds (`research` is an ARRAY with per-topic `sources[]`; `decide` carries `options[]` — replay-by-identity needs both; `present` is deliberately unrecorded); machine-truth, never prose. **FRAME-PHASE records (v13/v14, review-12 §III.3f, review-13 D): the v14 §3 amendment legalizes TWO more shapes with the SAME discriminant name (`kind`): `kind:'verify'` → `{cycle}` (NO `stepId` — a verify cycle belongs to the frame's verify phase) and `kind:'skip'` → `{stepId, condition, evaluated:false}` (NO `cycle` — a skip is step-keyed; NFR-OBS-1) — SIX kinds total (§8:291). The transcript makes replay deterministic: on replay the LLM calls are served the recorded completions and the interact calls the recorded answers **by question identity** — the human is NEVER re-interviewed and the questions never drift (review-7 N3). NOTE: this reverses the two-log split (op-log = metrics, never text; the transcript puts text in `events.jsonl`) — stated; NFR-SEC-1 (`no secrets in the tree`) now applies to prompt text (review-8 P12).
- **The replay/rework discriminator is TRANSCRIPT PRESENCE, not `ctx.feedback`** (review-8 P3 measured; review-9 confirmed): an attempt is NEW when no `trace` record post-dates the **ATTEMPT BOUNDARY** — the LATER of (the latest `rejected` event at the step's bound gate, the latest verify-cycle record) — where the bound gate is `confirm` for `at:'execute'` steps and `grill` for grill-bound steps; otherwise it REPLAYS (served from the transcript; a MISS falls through to a live call and appends at the next `seq` under the same `runId`). `ctx.feedback` marks the attempt kind (rework) and carries the rejection text. **`runId` = 1 + (rejections at the bound gate) + (verify cycles since the latest gate decision at `confirm`)** — the span the anchor names (§3 rule 8; review-13 A: "in the current attempt" is zero by construction since the boundary IS the latest verify record); derivable from the tail, monotonic; replay keeps the same runId (stranded partial records are served), a NEW rejection or a NEW verify cycle advances it (fresh records never collide). **The verify-cycle term applies to `at:'execute'` and `at:'confirm'` steps ONLY — a grill-bound step's boundary stays the latest `rejected` at `grill`** (review-13 B: a verify cycle re-enters at execute and must not move the grill phase's boundary, else the discriminator re-interviews a human at a confirmed gate). **This amends the review-10-settled runId derivation — a verify cycle is an ATTEMPT (review-12 §III.3a, measured: without the verify-cycle term, a verify retry REPLAYS and fails identically).**
...
- **§3 — the event schema**: `evidence` may carry the optional structured `trace` record (above — the transcript's home); **`submitted` may carry an optional structured `confirmedSha`** (the gate② content binding — recorded at `submit!(confirm)`, over marker-stripped content; review-8 P2, review-9 edit 5). **(v18 — journey-format-spec v18 §3: the SAME `evidence` event may carry the STRUCTURED CONCLUSION — `claims[]` = one `{ac, statement, evidence?}` per acceptance criterion (`evidence[]` entries are POINTERS — commit sha · ref path · doc name — resolved at READ time, never at write) and `checks[]` = `{command, result: 'pass'|'fail', detail?, sha?}` (what was RUN; `sha` binds the verification to the BYTES it ran against and must resolve in git). Both shapes are policed STRICTLY at the single writer — unknown key / empty ac / empty statement / a result outside pass|fail are refused by NAME; `complete!` then refuses a thin conclusion (an unclaimed AC, or no passing check bound to a cited commit) — see the intent row below and §4's verify.)**
...
|---|---|---|
| `evidence` | `{note, refs?, answers?[], claims?[], checks?[]}` — **(v18) `claims[]` = one `{ac, statement, evidence?}` per acceptance criterion (how it is met; `evidence[]` entries are POINTERS — commit sha · ref path · doc name — resolved at READ time, never at write); `checks[]` = `{command, result: 'pass'|'fail', detail?, sha?}` — what was RUN, `sha` binding it to the bytes it ran against (traceability: `checks[].sha` must resolve in git). Shapes + the strict write-time validation: journey-format-spec v18 §3; the conclusion is enforced at the CLOSE (§7 F-AC18)** | `append!` / `evidence!` → `evidence` (dedup per §2; the claim/check SHAPE is refused by name at the single writer — a bad `--claims`/`--checks` argument writes nothing) |
...
verify         outputs produced (materialized locks / evidence.commits[]) + ACs +
               step rules — the ACs read from the STRUCTURED conclusion (v18:
               evidence.claims[] + evidence.checks[] — journey-format-spec v18 §3),
               never from prose; judgment stays with the runner.
               FAILURE (v12-v14): re-run execute FRESH with the finding as
               feedback, up to flow.verifyFailCycles
               (config; default 1; ceiling 3) — anchor: since the latest gate decision
               at confirm (resets per rework pass); each cycle ADVANCES the attempt
               boundary (execute/confirm steps only); recorded as kind:'verify' {cycle};
               ceiling reached → failed. EMPTY CHAIN (flow 2): verify-fail = "waiting
               for the runner" — verifyFailCycles FORCED to 1 (context-inert) and the
               frame writes a `waiting` event → status blocked (resume state 4, §1).
               (Events record at commit, so verify checks OUTPUTS, not acceptance
               events.)
...
{ registry: 'config', version: 2, category: 'config', kind: 'registry',
  ...
  server: { host: '127.0.0.1', port: 8787 } }  // v5: the `ann serve` bind — a MACHINE
                                               // preference, env ANN_HOST/ANN_PORT
ONE CONFIG CLASS, TWO INSTANCES (review-12 §I.1b): the PROJECT file holds flow.*,
preferences.* and (v5) server.*; the USER overlay (~/.ann/config.json — the frozen
user-config schema, AMENDED to admit flow.*/preferences.*/server.*) overrides
preferences **and `flow.verifyFailCycles`** (a cost knob) — and (v5) the `server.*`
bind; `flow.conditionals` is project semantics and is NOT user-overridable
...
a project view; `ann config! set` gains the new keys (review-12 §I.1b.3) — (v5)
`ann config! set server.host|server.port`, with `ann config` showing each leaf's LAYER.
`ann serve` resolves its bind through THIS path (`--host`/`--port` per invocation outrank
every layer) and REFUSES on a named `server.*` problem — never a clamp, never a silent
fall-back.
...
- **Chains are LINEAR by default** (`flow.conditionals: false`). **Setting `flow.conditionals: true` enables `when?` on chain entries** — linear-with-optional-skips; a runtime branch is then DATA, not code (owner decision 2026-08-26). **THE NEW BOUNDARY, STATED HONESTLY (review-12 §V): skip-shaped only — no else-branch, no alternative step, no join; TWO fixed predicates — a THIRD condition kind is a CODE change (review-13 F: `inputMissing` was the third; edit 6 dropped it this round); NO conditional gate sources (rule 6); the flag is PROJECT-WIDE (one boolean for every chain in the project).** `conditionals: false` + a chain carrying `when` → FAIL CLOSED, named (flow.ts:76's standard; review-12 §II.2e). The GATE reject bound and gate set/positions are untouched by the flag.
...
8. **Amendments recorded BEFORE implementation** (each: complete superseding artifact, same logical name, preserving section anchors, back-reference, `superseded` event, referrer re-pointing):
   - **journey-format v13 → v14**: §2 (workType/flow/model; node.json schema incl. top-level `openQuestions`; REQUIRED set) · **§3 (the `trace` record on `evidence` — the transcript's home, shape-policed for SIX kinds: the four step-level kinds `llm|ask|research|decide` + the frame-phase `kind:'verify'` `{cycle}` and `kind:'skip'` `{stepId, condition, evaluated:false}` — review-13 D; the gate②-confirmed sha record; the NEW `waiting` event kind — review-13 E)** · **§4 (:90 "artifact-locked is written the moment the artifact locks" + :87/:88 write-timing — DEFER-RECORD deviates; recorded as the amendment's own rationale)** · §14/§15 (the `-v<N>` filename rule is ADDED, not amended — v13 has none); the `store.spawn` write + `context.ts` read changes are OWNED by this ship list (not "unchanged")
   - **journey-format v17 → v18**: §3 (the STRUCTURED CONCLUSION on `evidence` — `claims[]` = one `{ac, statement, evidence?}` per acceptance criterion, its `evidence[]` entries POINTERS resolved at READ time; `checks[]` = `{command, result: 'pass'|'fail', detail?, sha?}`, `sha` binding the verification to the bytes and required to resolve in git; the strict write-time validation, refused by NAME at the single writer — journey-format-spec v18 §3) · §7 (the v18 conclusion gate: `complete!` refuses an unclaimed AC or no passing check bound to a cited commit — the ONE structured write-path refusal beside the post-hoc F-AC18 checks) · §9 (the traceability rule extends to `checks[].sha`) · §14 (the structured conclusion beside the commit clues). The code landed first (d2dd59a · fe1c74b); this records it.
