## Link contract
- **upstream** (this doc relies on): tree/rounds/01-goal/artifacts/design.md,02-grilling/01-spec-rework/artifacts/requirements-spec.md 05-engine/00/09-format-amendment-v4/artifacts/tree-format-spec-v4.md,docs/architecture.md
...
### Tech stack — decided with `how: discussed` (resource center: rules/ask system-design rung #6)

- **TS-1 Language/runtime:** TypeScript on Node.js. *how: discussed* — re-elicited from the inherited skeleton (the tech-stack lesson); alternatives reviewed (Python, Go/Rust); user confirmed.
- **TS-2 CLI:** thin, framework-light (commander or bare node: parsing); F1–F17 surface in `src/surface/`, swappable. *how: discussed*.
- **TS-3 Tests:** vitest (TS-native; the S9 fixture suite depends on it). *how: discussed*.
- **TS-4 Build/run:** tsx for dev, tsc for build; no bundler in v1. *how: discussed*.
- **TS-5 Package manager:** npm (pnpm acceptable). *how: discussed*.
- **TS-6 LLM provider:** adapter is the boundary; first adapter = thin fetch-based client against an OpenAI-compatible endpoint; per-task model selection. *how: discussed*.
- **TS-7 Web-UI constraint:** CLI ships v1; human-interface contract stays clean so a web surface (plain HTML or React later) wraps the same adapter. *how: discussed*.

### Q1–Q4 provenance (re-labeled honestly, per rules/decide)

- Q1 runtime → **how: discussed** (originally defaulted from the skeleton; re-elicited + user-confirmed in the Q1–Q4 re-ask round).
- Q2 provider → **how: discussed** (adapter-first; per-task models added by conformance; user-confirmed in the stack round).
- Q3 storage → **how: discussed** (the no-SQLite deviation was flagged + user-informed; file-tree confirmed).
- Q4 UI → **how: discussed** (CLI v1 confirmed; web UI = target, v2).
