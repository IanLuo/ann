## Link contract
- **upstream** (this doc relies on): journey/legs/01-goal/artifacts/design.md,02-grilling/01-spec-rework/artifacts/requirements-spec.md,docs/tree-format-spec.md,docs/core-design.md,docs/journey-format-spec.md
...
# Journey Format Spec (v18)
*Artifact of task `10-server-ui/05-implementation-journey-format-v18`. Type: spec. Complete superseding version — v17 (locked @ 12e9c14) + **the STRUCTURED-EVIDENCE AMENDMENT (leg 10 task 05)** — the recorded half of the landed structured-verification change (the code landed first: d2dd59a structured evidence · fe1c74b the conclusion gate): an `evidence` event may carry TWO more OPTIONAL structured fields, so a gate reading is decidable from the log instead of from prose. **§3** enumerates both with their exact shapes — `claims[]` = one `{ac, statement, evidence?}` per acceptance criterion (how the AC is met; `evidence[]` entries are POINTERS — a commit sha, a ref path, or a doc name — resolved at READ time, never at write) and `checks[]` = `{command, result: 'pass'|'fail', detail?, sha?}` (what was RUN; `sha` binds the verification to the BYTES it ran against) — with the STRICT WRITE-TIME validation (unknown key / empty ac / empty statement / a result outside pass|fail refused by NAME) and the READ-time pointer-resolution rule; **§7/§14** carry the v18 conclusion gate (the close refuses an unclaimed AC or a conclusion with no passing check bound to a cited commit); **§9** extends the traceability rule to `checks[].sha`; **§11** records the amendment; section anchors preserved. HISTORY (v17 — the version this supersedes): Artifact of task `10-goal-record/01-implementation-goal-session-record`. Type: spec. Complete superseding version — v16 (locked @ 047f1b2) + **the GOAL-SESSION RECORDED AMENDMENT (leg 10)** — the recorded half of the landed goal-scoped-sessions change: a journey is ONE GOAL; the goal is a designated **SEEDED CHILDLESS leg**, the single leg-root exception to the v7 no-events rule. **§1/§4/§7/§12/§17** — that goal leg may carry root events: the seed (`created`+`completed`, written only by `seedGoal`, never the general writer), the artifact-locked record for goal.md (artifact name `goal`), and the new `goal-met` eventType — nothing else; **§3/§17** — `goal-met` (the HUMAN met verdict, status-inert, guarded) is NEW in the event-type list; **§12/§17** — the seeded childless goal leg derives `done` WITHOUT tasks (its seed events make `legStatus` done) so the first work leg's `legGateMet` opens; **§17** — goal.md is the DOC SOURCE OF TRUTH (fixed `Goal:`/`Success criteria:` sections, artifact-locked on the goal root) and the goal node.json is machine-generated 1:1 from it; `goal! met` seals the verdict, `goal! archive` is the guarded structural reset to `.ann/archive/sessions/<ts>-<slug>/journey/` (the second designated cross-folder writer). The code half landed first (the goal-scoped-sessions commit); this v17 records it. Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v17 (locked @ 12e9c14), `core-design` (locked @ 4647da7). Referrers: flow-control-spec · change-protocol · ann-system-design · architecture · AGENTS.md · implementation slices.*
...
- Chain order = leg index; a leg's "goal met" = its epic ACs verified (all tasks' goals met + artifacts locked) → **leg status derives `done` from its tasks (§12)** → next leg spawns. **(v7: the leg root carries no events — completion is derived from tasks, never asserted. **v17 §17: the designated seeded childless goal leg is the ONE leg-root exception.)**
...
- One JSON object per line. Types: `created · activated · extended · evidence · artifact-locked · completed · failed · superseded · submitted · confirmed · rejected · waiting · goal-met`. (v4: **`spawned` removed** — a child's `created` event IS the spawn record; children are structural via directories.) **(v14: `waiting` is NEW — below.)** **(v17: `goal-met` is NEW — §17, a goal-root verdict.)**
...
- **`goal-met` — the goal verdict (v17 — NEW; §17).** `{"at","type":"goal-met","decision":"met","feedback":"optional string","note":""}` — the recorded HUMAN verdict that a structurally-exhausted session's success criteria are met (`goal! met`). Recorded **ONLY on the designated childless goal leg root** (§17) — refused on any task id and on any non-goal leg. **Status-inert**: it has no tail-mapping case — it annotates, never overrides a status (a `done` goal leg stays `done`), never satisfies a gate, never concludes anything. Guarded at the command: human initiator only (refuses an automated `RECORDED_BY`), no double-met, refused while any undecided submission exists, no post-met spawns. Written by `goal! met` only — the general writer never emits it.
...
- **Structured event fields:**
  ...
  - **`evidence` may ALSO carry the STRUCTURED CONCLUSION (v18 — NEW): `claims[]` + `checks[]` (the code landed first: d2dd59a · fe1c74b).** Two more OPTIONAL structured fields on the SAME `evidence` event — the conclusion made machine-readable, so the gate reviewer (and the close, §7 F-AC18) reads the LOG instead of prose.
    ```json
    {"at":"…","type":"evidence",
     "commits":[{"sha":"d2dd59a","note":"structured evidence + the read-time pointer resolver"}],
     "claims":[{"ac":"AC-1","statement":"docs/journey-format-spec.md enumerates both fields with their exact shapes",
                "evidence":["d2dd59a","docs/journey-format-spec.md"]}],
     "checks":[{"command":"npm test","result":"pass","detail":"634 passed / 37 files","sha":"d2dd59a"}],
     "note":""}
    ```
    - **`claims[]`** = `{ac, statement, evidence?}` — **one per acceptance criterion**: `ac` names the criterion it speaks to (its `AC-N` id **or the criterion's own text**), `statement` says HOW it is met, and the OPTIONAL `evidence[]` is a list of **POINTERS** — each entry a commit sha, a ref path, or a doc name.
      - **POINTERS RESOLVE AT READ TIME, NEVER AT WRITE.** The writer stores the pointer strings verbatim; a view resolves each one — a 7–40-hex sha via `git cat-file -t` → `resolves in git`; else a project-relative path (a file → its line count, a directory → `directory`); else the docs manifest / a current artifact → `<path> @ <sha>`; otherwise `UNRESOLVED — no commit, path or doc`. A pointer that stops resolving IS the finding: it is REPORTED, never dropped — and never refused at write time (pointing at a commit that does not exist YET is legal to record).
    - **`checks[]`** = `{command, result, detail?, sha?}` — **what was RUN and how it went**: `command` = the exact command; `result` ∈ `'pass' | 'fail'`; OPTIONAL `detail` = the evidence of the result (a count, an excerpt, a finding); OPTIONAL `sha` = the commit whose BYTES the check ran against.
      - **THE TRACEABILITY CLAUSE (v18):** a `checks[].sha` must resolve in git (`git cat-file -t <sha>` succeeds). `sha` is OPTIONAL — a check recorded without one asserts a run but binds no bytes — but a check whose sha does NOT resolve is a verification claim about nothing, so `check()` reports it by name (F-AC18, §7).
    - **STRICT WRITE-TIME VALIDATION — NAMED, fail-closed** (the single writer; `evidence!`/`append!` REFUSE and write nothing): `claims`/`checks` must each be a NON-EMPTY array of objects; an **unknown key** on either is refused (claims allow only `ac` · `statement` · `evidence`; checks only `command` · `result` · `detail` · `sha`); a claim with a missing/blank **`ac`** or a missing/blank **`statement`** is refused; a claim's `evidence`, when present, must be a non-empty array of non-blank strings; a check with a missing/blank `command`, a **`result` outside `pass|fail`**, a non-string `detail`, or a malformed `sha` (not 7–40 hex) is refused. Each refusal NAMES the field — the shape lives in the ONE writer, and the CLI's `--claims`/`--checks` are a thin validated front over it (bad JSON or a non-array argument fails closed, writing nothing).
    - **HOW THE CONCLUSION IS READ (v18).** A claim is lined up with its contract AC by identity — the criterion's `AC-N` (case/whitespace-insensitive) or a PREFIX of the criterion's own text — and the LATEST claim for a criterion wins (a later claim supersedes an earlier one for the same `ac` — the rework model). An AC left unclaimed is reported **`NO CLAIM RECORDED`**; a claim that matches NO AC is KEPT (an author's extra claim is data, never dropped); checks are the run history in append order; the cited commits (`commits[].sha`) are the task's conclusion set. `complete!` REFUSES a close when any contract AC is unclaimed, or when no check is both `result:'pass'` and bound (its `sha` ∈ the cited commits) — §7 F-AC18.
...
| `events.jsonl` | leg root = **— (absent — v7)**; **v17 §17: the seeded childless GOAL leg's root carries its OWN `events.jsonl` (the seed, the goal.md lock, `goal-met`)** · task = **task lifecycle** (`created`-as-spawn-record/`activated`/`evidence` (with optional structured `commits`/`refs`/`trace`/`claims`/`checks`)/`artifact-locked`/`completed`/`failed`/`superseded`/`waiting` + gates + closure events) | **appended only**; `artifact-locked` on the **producing node** (L1's root holds design's — grandfathered; tasks hold theirs). **(v7) The leg root carries no process log** (v17 §17: the goal leg excepted) · **(v14) the `artifact-locked` event records AT COMMIT** — below |
...
- **F-AC17 (v7) — leg-status invariants:** the validator derives leg status from children: all tasks `done` → leg `done`; else the leg's status = the frontmost-ready child's; no tasks → `queued` (a leg without tasks can never be `done`). A leg root carrying ANY event (new legs; pre-v7 roots grandfathered, §12) = validator error. **(v17 §17: the designated seeded childless goal leg is the one carved-out exception — it may hold the seed, the goal.md lock, and `goal-met`, nothing else.)** **Gate/closure rules (F-AC15/16) apply to TASKS only — leg roots are skipped**, so legacy root events can never trip or contradict gate checks.
- **F-AC18 (v9; v10 amended) — conclusion invariants:** every completed TASK (spawned under v9+) must conclude with **a locked artifact (document) OR structured commit evidence** — an `evidence` event with a non-empty `commits[]` (the machine-truth record of the task's output, §3/§14). A task that produced nothing records the reason in its completed note or an evidence event. The one-time migrations (§15) are exempt. **(v14: an `evidence` event carrying ONLY a `trace` record does NOT conclude a task — the transcript is process, not product; and a `waiting` record concludes nothing.) (v18 — the CONCLUSION GATE: a close is also a REVIEW. `complete!` refuses, NAMED and writing nothing, a task whose contract ACs are not all claimed (`claims[]`, §3) or whose conclusion holds no `result:'pass'` check bound to a cited commit (`checks[].sha` ∈ `commits[].sha`). The refusal reads the SAME derivation the card renders — the card can say `NO CLAIM RECORDED` and the close then refuse, never one without the other.)**
...
- **Commit traceability (v10):** structured `commits[]` must resolve (`git cat-file -t <sha>` succeeds) and `refs[]` paths exist; unresolvable → validator error. **Exemption: the one-time migrations (§15) — pre-v10 artifacts whose commit history is untraceable are skipped, never failed.** **(v18: the same traceability covers `checks[].sha` — a check whose sha does not resolve is a verification claim about nothing, and the validator reports it by name.)**
- **A claim POINTER that stops resolving (v18):** a READ-time finding, never an error. The card prints `UNRESOLVED — …` beside the pointer; the write was never refused and the pointer is never dropped — the pointer that stopped resolving IS what the reviewer must see (§3).
...
- **v17 records the goal-session change (leg 10 — the recorded half).** The code half landed first (the goal-scoped-sessions commit — `goal`/`goal!`/`next`, the `goal-met` event type, `seedGoal`, `archiveJourney`); THIS amendment records it. The designated SEEDED CHILDLESS goal leg is now the single leg-root exception to the v7 no-events rule — the format's load-bearing invariant (§1/§3/§4/§7/§12) is amended in one place (§17) rather than carved ad hoc: the goal root may hold the seed, the goal.md lock, and `goal-met`, and nothing else. `goal-met` is a new protected eventType (status-inert). goal.md is the doc source of truth; the goal node.json is machine-generated 1:1 from its `Goal:`/`Success criteria:` sections. `goal! met`/`goal! archive` add the guarded met verdict and the cross-folder archive move. Section anchors preserved; §17 appended.
- **v18 records the structured-evidence change (leg 10 — the recorded half).** The code half landed first (d2dd59a structured evidence · fe1c74b the conclusion gate); THIS amendment records it. A gate could only be decided from PROSE: the conclusion was a `note`, and "how is AC-3 met, and what was actually run?" lived outside the model. Two OPTIONAL `evidence` fields make the conclusion machine-readable — **`claims[]`** (one per acceptance criterion, `ac` = the criterion's id or its text; its `evidence[]` entries are POINTERS resolved at READ time, so a pointer that stops resolving is the FINDING) and **`checks[]`** (what was RUN; `sha` binds the verification to the bytes it ran against and must resolve in git). The single writer polices both shapes strictly, by name (§3); the close polices the conclusion (§7 F-AC18). §3/§4/§7/§9/§14 amended; section anchors preserved.
...
| no children (nothing spawned) | own lifecycle from root events — grandfathered childless legs only (L1, the base step); new legs carry no root events → **queued**, never `done` **(v17 §17: the SEEDED childless goal leg is the exception)** |
| the SEEDED childless GOAL leg (v17 — §17) | **done** — derived from its root seed events (`created`+`completed`, written by `seedGoal`), so the first work leg's `legGateMet` opens; the goal.md lock and `goal-met` ride the same carve-out, nothing else |
...
- **structured conclusion (v18) — `claims[]` + `checks[]` (§3).** For a task whose contract has acceptance criteria, the conclusion is also the structured one: one claim per AC (how it is met, with POINTERS resolved at READ time) and the checks that were RUN (`result` + the optional `sha` binding the verification to the bytes). OPTIONAL to the FORMAT, enforced at the CLOSE: `complete!` refuses an unclaimed AC or a conclusion with no passing check bound to a cited commit (§7 F-AC18).
- **conclusion** — what the task concluded, in the `completed` event note or an evidence event (v18: the structured form above when the contract has ACs). A task that produced nothing records the reason; a locked artifact is not forced.
...
*(Historical note: `27-journey-format-v15` — the task producing v15 — was named legacy-style (no worktype) because it was the last task spawned under the v14-era 24-char cap; a worktype-tagged proof-of-concept name (`27-implementation-journey-format-v15`, 35 chars) was impossible before the cap raise that very amendment performed. The grammar applied from the next task onward; the v17 producer (`10-goal-record/01-implementation-goal-session-record`) is grammar-compliant.)*

## 17. The goal leg — the ONE leg-root exception (v17 — NEW)

**What this records.** The goal-session change (goal-session-design v6; the code half landed 2026-09-05 in the goal-scoped-sessions commit). One journey = **ONE GOAL**; the goal is the root **goal leg**, and it is the format's single leg-root exception. Everything in this section is the amendment's delta; the affected rules (§1/§3/§4/§7/§12) carry pointers here.

**The designated childless goal leg.** The goal is not "the frontmost leg" and not any `01-goal`-named leg — a `01-goal` that carries TASKS is an ordinary work leg. The goal leg is the leg that is **childless AND carries the goal seed/artifact** (the engine's goal predicate): a childless leg whose root holds the `created`+`completed` seed or a locked goal.md. Identity is **structural, never the name**.

**Its allowed root events — nothing else.** The goal leg root may carry exactly: (1) the **SEED** — `created` + `completed`, written only by `seedGoal` at `goal! seed`, never through the general writer; (2) the `artifact-locked` record for goal.md (artifact name `goal`); (3) the `goal-met` verdict. Everything else on a leg root stays refused (§3), and `goal-met`/the goal.md lock are refused on every other id (tasks and non-goal legs).

**Done-without-tasks (why the seed exists).** A leg with no tasks can never derive `done` (§12) — so an unseeded goal leg would leave the whole session `queued` and the FIRST WORK LEG's `legGateMet` (the engine leg gate) closed. The seed (`created`+`completed` on the goal root) is what makes `legStatus` derive `done`, opening the first gate. Status-inert alone would hard-block all work.

**`goal-met` (v17 — NEW eventType, §3).** `{"at","type":"goal-met","decision":"met","feedback":"optional string","note":""}` — the recorded HUMAN verdict that a structurally-exhausted session's success criteria are met (`goal! met`). **Status-inert**: it has no status-derivation case — it annotates the goal root, never overrides a status, never satisfies a gate, never concludes a task. Guarded at the command: human initiator only (an automated `RECORDED_BY` is refused), no double-met, refused while any undecided submission exists anywhere, no post-met spawns.

**goal.md — the DOC source of truth.** goal.md carries fixed `Goal:` / `Success criteria:` sections, is authored by grilling (a session begins by grilling the goal), and is **artifact-locked on the goal leg root** (hash-checked immutability). The goal node.json is **machine-generated 1:1** from the doc — `Goal:` → `contract.intent`, the `Success criteria:` bullets → `contract.acceptanceCriteria` — and the machinery (`goal`/`next`/`goal! met`/context packets) reads the generated node, never a hand-authored duplicate. The goal is **immutable per session**: a changed goal is a NEW session (archive → re-grill), never an in-place edit.

**`goal! met` | `goal! archive`.** `goal! met` records the sealed verdict. `goal! archive` is the guarded structural reset: it moves `.ann/journey/legs/*` + the write-rev ledger to `.ann/archive/sessions/<ts>-<slug>/journey/{legs, .ledger.json}` — a layout that round-trips `Store()` so an archived session loads READ-ONLY — removes the live ledger entry, and resets the live store to an empty session (no goal → MISSING). `goal! archive` is the **second designated cross-folder writer** (after `supersede!`); it is named in AGENTS.md before it existed (write confinement).
