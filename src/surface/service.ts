 ...
 * `goal!`/`spec!`/`archive` are NOT routes, and the refusal is a named error doc
...
const notExposed = (name: string): Outcome => ({
  ...
  error: {
    code: 'not-exposed',
    message: `serve: '${name}' is not exposed — the minimal slice is journey · status · next · detail · confirm · results · packet · gates (GET) and the gate write (POST /api/gate). run!/spawn!/submit!/goal!/spec!/archive are deliberately absent (scope OUT).`,
  },
});
...
async function route(root: string, req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (name !== 'gates' && !READ_ROUTES.has(name)) return sendJson(res, 404, jsonDoc(notExposed(name)));
