...
export { loadConfig, setConfig, maskedConfig, configPath, configExists, resetConfigCache, listProjects, getCurrentProject, setProject, useProject, removeProject } from './config.js';
...
export function getAdapter(provider?: string, logRoot: string = process.cwd(), corr: { traceId?: string; runId?: string } = {}): ProviderAdapter {
  ...
  return new OpenAICompatibleAdapter(entry, reg.defaults, logRoot, corr);
}
