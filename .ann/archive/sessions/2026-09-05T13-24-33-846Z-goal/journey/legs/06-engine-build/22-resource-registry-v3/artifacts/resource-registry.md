<!-- specs:locked:e2dfb46 2026-08-26 type=spec -->
## Link contract
- **upstream** (this doc relies on): 04-system-design/00/10-system-design-amendment/artifacts/ann-system-design-v2.md,05-engine/00/07-architecture/artifacts/architecture.md,06-engine-build/19-core-design/artifacts/core-design-spec.md
- **referrers** (must cite this when they change): implementation slices,validators the specs ritual,review-task

# Resource / Registry Spec (v3)
*Artifact of task `06-engine-build/22-resource-registry-v3`. Type: spec. Complete superseding version — v2 (locked @ 4f95b5a) + the **`schema` and `config` categories**, the **`vocab` instance row**, and the **code-literal-duplicate RECONCILIATIONS** (`core-design` §1/§6/§8, locked @ f7fb400). What changed: §3's category list gains **`schema`** (the vocabulary registry that event types, statuses, gates and artifact types stand on — it existed on disk as `rules/schema/vocab.json` while the spec's frozen list did not admit it) and **`config`** (the general user-facing configuration — ONE CONFIG CLASS, TWO INSTANCES: the per-project `rules/config/default.json` and the per-user `~/.ann/config.json` overlay); §4 gains the **vocab** and **config** instance rows; §5 gains the **reconciliation register** — the places where a registry value is KNOWINGLY duplicated as a code literal, recorded rather than pretended away; and §8's frozen user-config schema is **AMENDED to admit `flow.*` / `preferences.*`**. This also **RECONCILES `architecture` v2:84**, which listed the registries as "vocab · decide/ask/check · adapter/provider" while this spec's frozen category list had no `schema` category to put `vocab` in. §9 (NEW) **LISTS the data migrations the re-implementation must execute — listed here, executed there.** Upstream: `ann-system-design` (v3, locked @ 2b0a30f), `architecture` v3 (locked), `journey-format-spec` v14 (locked), `core-design` (locked @ f7fb400). Referrers: implementation slices, validators, the specs ritual, review-task.*
...
**THE RECONCILIATION REGISTER (v3 — NEW).** A registry value that is ALSO a code literal is a **knowing duplicate**. Each one is recorded here with the reason it must be a literal and the consequence of editing the data alone. Nothing may be added to this list silently — an unrecorded duplicate is a defect.
