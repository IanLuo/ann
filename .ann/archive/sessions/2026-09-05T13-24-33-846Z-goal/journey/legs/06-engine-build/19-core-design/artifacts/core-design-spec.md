<!-- specs:locked:f7fb400 2026-08-26 type=spec -->
...
| **the general config registry** (`rules/config/default.json`) | **data, adjustable** | the user-facing knobs (conditionals · verify cycles · ask-vs-assume · defaults); **precedence `env > user > project > builtin`, per leaf key — `flow.conditionals` is NOT user-overridable** (project semantics; the product IS the journey — review-13 F); read via the resource-registry pattern — never hardcoded |
...
schema, AMENDED to admit flow.*/preferences.*) overrides preferences **and `flow.verifyFailCycles`** (a cost knob) — `flow.conditionals` is project semantics and is NOT user-overridable
