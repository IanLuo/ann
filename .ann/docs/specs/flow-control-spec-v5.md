...
# Flow Control Spec (v5)
*Artifact of task `06-engine-build/13-format-amendment-v9`. Type: spec. Complete superseding version — v4 (locked @ 43beff2) + the artifact completion rule (every task concludes with a lockable artifact; implementation artifacts are commit-record docs; the §6 "artifact recorded" requirement is now universally satisfiable — resolves the §6/§7 contradiction). Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v9 (locked). Referrers: S5 planner kernel, implementation slices, review-task.*
...
- **commit:** checkpoint (git), **artifact recorded** — the task's artifact per journey-format-spec v9 §14 (a document, a commit-record doc, a link, or a reason doc; every task concludes with one). Gates enforced: **leg gate** (leg N+1 only after all of leg N's tasks are `done` — the derived aggregate, format v7 §12), **artifact gate** (children only after the node's artifact exists — satisfiable by any locked artifact, incl. a commit-record doc), **completion** (ACs + evidence + GATE② accepted).
...
- `done` ⇔ ACs verifiably met + evidence + **artifact recorded** + GATE② accepted. **(v5) "artifact recorded" is now universally satisfiable** — every task locks an artifact at completion (a document, a commit-record doc, a link, or a reason doc — journey-format-spec v9 §14), so the rule holds for *every* work type, including implementation.
...
- Work types parameterize the common skeleton (materialize / missing-input / verify / chain effect). **(v5) The artifact column is guidance, not a constraint — a task produces whatever it produces (journey-format-spec v9 §14); every task concludes with a lockable artifact.**
...
| implementation | probe/derive aggressively | infer w/ fallback; ask high-impact | tests + evidence; **artifact = commit-record doc (refs + commit clues + tests; presupposition: task code is well-committed and described)** | commit → next frontmost |
...
- Immutable once locked. Amendments = **new nodes** whose artifacts **completely supersede** this spec (back-reference + `superseded` event). v5 resolves the v4 §6/§7 contradiction: §6 demanded "artifact recorded" for completion while §7's implementation row verified by "tests + evidence" with no lockable artifact — leaving code tasks unable to satisfy the artifact gate. The v9 artifact model (journey-format-spec §14) makes the artifact universal and the two sections consistent.
