...
- `scripts/validate.mjs` — the rule registry report (rules/check/rules.json); runs the active rules — the S4 validators seed, kept until absorbed. Both scripts consume the **vocab registry** (`rules/schema/vocab.json` — event types, statuses, gates, artifact types; resource-registry spec instance #2); no layer hardcodes vocabulary.

## Migration note (2026-08-19)

The store was renamed `tree/rounds/` → `journey/legs/` (unit: round → leg; logical name: tree-format-spec → journey-format-spec) and flattened (`leg/00/task` → `leg/task`). **All compat symlinks (`tree → journey`, `journey/rounds → journey/legs`, and the `leg/00 → .` links) were removed the same day** — legacy recorded paths (the design event's `tree/rounds/…` and the v7/v4 locks' `…/00/…`) normalize to the current layout in the resolver (`legacyPath`), so the store is exactly one folder: `journey/` containing `legs/` and nothing else. Historical references in locked docs may still cite old paths — they are history, not resolution inputs.
