<!-- specs:locked:71fa442 2026-08-26 type=spec -->
## Link contract
- **upstream** (this doc relies on): journey/legs/01-goal/artifacts/design.md,02-grilling/01-spec-rework/artifacts/requirements-spec.md,05-engine/17-format-amendment-v6/artifacts/tree-format-spec-v6.md,06-engine-build/19-core-design/artifacts/core-design-spec.md
...
# Journey Format Spec (v14)
*Artifact of task `06-engine-build/20-journey-format-v14`. Type: spec. Complete superseding version — v13 (locked @ 699615a) + **the four recorded core-design amendments** (`core-design` §2/§3/§8, locked @ f7fb400): **§2** — the `contract` schema gains `workType` · `flow` · `model`, and `openQuestions` is stated as a **TOP-LEVEL SIBLING** of `contract` (as v13's own schema block already had it), with the REQUIRED set restated; **§3** — the event schema gains the structured **`trace` record on `evidence`** (the transcript's schema-legal home — SIX kinds: the four step-level `llm|ask|research|decide` + the frame-phase `verify` `{cycle}` and `skip` `{stepId, condition, evaluated:false}`), the optional **`confirmedSha` on `submitted`** (the gate②-to-commit content binding), and the **NEW `waiting` event kind** (maps to `blocked` in the status derivation); **§4** — write timing becomes **DEFER-RECORD**: the working file materializes the moment the step writes it, but the `artifact-locked` EVENT (and deferred child spawns) record **AT COMMIT**, after gate② confirms — this deliberately deviates from v13 §4's "`artifact-locked` is written the moment the artifact locks", and the rationale is recorded in §4/§11; **§14/§15** — the shared-document **`-v<N>` filename rule** is ADDED (v13 stated no filename rule; the convention existed only in practice). Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v13 (locked @ 699615a), `core-design` (locked @ f7fb400). Referrers: flow-control-spec · change-protocol · ann-system-design · architecture · AGENTS.md · implementation slices.*

## 1. Layout — the table of legs

- **THE PROJECT LAYOUT (v12): all ann-owned files live under `<project>/.ann/`** — `journey/` (the store) · `docs/` (shared contract-stack content) · `rules/` (registries). Subtle by design: ann-owned files are MACHINE-managed (never hand-edited — the invariants forbid it); users interact through ann (CLI/UI), not the raw files. Root-level `journey`/`docs`/`rules` may exist as **compat symlinks → `.ann/*`** (historical recorded paths, external references, relative artifact links) — the walkers skip symlinks, so the old shape is invisible to state. **Recognition marker: a directory containing `.ann/` IS an ann project** (legacy: `journey/` is also accepted).
...
7. **Work type** — SHOULD be **stated in `contract.workType`** (v14 — v11 said "determinable from intent"; the flow now reads a field, so state it): validate / envision / spec / implementation / binding / closure (flow-control §7). Determinability is no longer enough where a chain must be selected.
...
- **Structured event fields:**
  - `artifact-locked` carries the artifact's durable identity: `{"at","type":"artifact-locked","artifact":{"name":"requirements-spec","path":"…/requirements-spec.md","lockSha":"89ace76"},"note":""}`. **`path` is the task-local ref** — a symlink into `docs/` (shared document artifacts) or the real file in `artifacts/` (task-local artifacts, e.g. a goal or report). **`lockSha` remains the blob sha of the file content** (doc integrity), computed over **MARKER-STRIPPED content**. **(v14: the EVENT records at COMMIT — §4. The file exists earlier; the event is the acceptance record, not the write record.)**
...
| `artifacts/` | node outputs — **a ref (symlink into `docs/`) for SHARED document artifacts (specs, designs, architecture); a REAL file for task-local artifacts (a goal, a report, a grandfathered record)** (§14/§15). **Shared content lives in `docs/`, never in the tree** (§15); task-local content lives in the task's `artifacts/` | the **WORKING FILE** is written when the step produces it (task-local, re-written on rework); the **shared `docs/` placement + symlink + version number** happen **at COMMIT** (§14) |
| `docs/` | the **SHARED artifact content files** — the contract stack only, categorized by type, `-v<N>`-named (§15) | materialized at COMMIT, byte-verbatim from the working file |
...
- **THE `artifact-locked` EVENT RECORDS AT COMMIT** — after gate② (`confirmed(gate=confirm)`) — together with the shared `docs/<category>/<name>-v<N>.md` placement, the symlink, and the choice of `N` (§14/§15). **Deferred child spawns record there too** (both are acceptance-shaped writes; the commit phase orders **locks before spawns**, so a child's `requiredInputs` always resolve via `current()` when its spawn runs).
...
- **Two shared artifacts whose `-v<N>` would collide (v14, §15):** the commit phase picks `N` = (highest existing `N` for that logical name) + 1, derived from the recorded `artifact-locked` paths — never from a counter and never from the filesystem alone.
...
- **file ref** — a path into the project. **SHARED document artifacts** (specs, system-designs, architecture — the contract stack) live in `docs/`, categorized by type (§15), named `<name>-v<N>.md` (the filename rule, below), realized in the task's `artifacts/` as a **symlink**. **TASK-LOCAL artifacts** (a goal, a report, a record) are **real files** in the task's `artifacts/`. **External products** (outside the project) are referenced by a **link** (URL) — no file existence check.
...
**THE FILENAME RULE (v14 — ADDED, not amended; core-design §3 rule 7).** v13 stated *where* shared content lives (§15) but never *what it is called*. The rule, now contractual:

- **A shared document artifact's content file is `docs/<category>/<logical-name>-v<N>.md`** — `<category>` from the artifact TYPE (§15), `<N>` a monotonically increasing integer, starting at the version the logical name is on when it first lands in `docs/`.
- **`N` is chosen AT COMMIT** (§4 defer-record) as (highest recorded `N` for that logical name) + 1, derived from the `artifact-locked` paths in the log — never from a counter, never from the filesystem alone.
- **The task-local ref keeps the bare logical name**: `artifacts/<logical-name>.md` → the versioned file. Referrers therefore never carry a version in their path.
- **Placement, category and the versioned flag derive from the artifact's TYPE** via the vocab registry — a new type in an existing category is DATA; a new `docs/` category is a §15 amendment.
- **TASK-LOCAL artifacts are NOT versioned** — they are real files named for what they are (`goal.md`, `s2-record.md`, `flow-control-deferrals.md`). Superseding a task-local artifact means a new task with a new file, as always.
- **Historical exception, recorded:** files that landed in `docs/` before the rule was written and carry no `-v<N>` (`flow-control-spec.md`, `requirements-spec.md`, `tree-format-spec.md`, `resource-registry-spec.md`, and the other v1s) stand as recorded — their `artifact-locked` paths are facts. The rule governs new files; it never rewrites history.
...
- **Working files are not artifacts (v14).** A file a step wrote but no `artifact-locked` event records is **working state** — task-local, unpoliced, outside the integrity model, never promoted to `docs/`. Acceptance is what makes a file an artifact.

## 15. docs/ layout & the migrations (v9 — NEW; v10 amended; v12 amended; **v14 amended**)

**Where shared content lives.** `docs/` (i.e. `.ann/docs/`) holds **only the SHARED contract stack** — docs cross-referenced by logical name across the journey — categorized by artifact type, one place per file, **`-v<N>`-named (§14)**; git tracks changes; the tree holds refs (symlinks) + events. **No catch-all category.** Everything else is task-local and lives in the task's `artifacts/` (real files).

| Category | Contents | Filename |
|---|---|---|
| `docs/specs/` | specs and policies (requirements-spec, format specs, flow-control, functional, resource-registry, change-protocol, depth-policy, core-design) | `<name>-v<N>.md` |
| `docs/system-designs/` | system designs (design, ann-system-design*) | `<name>-v<N>.md` |
| `docs/architecture/` | architecture | `<name>-v<N>.md` |

*(v10: `docs/records/` was REMOVED — it was a catch-all holding goal.md + conformance-report.md + s2-record.md, which are task-local, not shared contract stack. They returned to their tasks' `artifacts/` as real files; their `artifact-locked` events already point at those paths, so no event changes.)*

- **Adding a `docs/` CATEGORY is a §15 amendment** (this section is the list). Adding an artifact TYPE inside an existing category is **registry data** (the vocab entry carries its category + versioned flag) — no amendment needed. **(v14 — the split core-design §3 rule 7 names.)**
- **Updating a shared file needs no reference resolution** — the task-local symlink still points at the same `docs/` path; the commit advances, the ref does not.
- **Migration (2026-08-21, sanctioned — v9 hot fix).** 27 artifact content files moved from `journey/legs/…/artifacts/` to `docs/` (categorized); a symlink replaces each at its old path, so every recorded `artifact-locked` path and blob sha keeps resolving — append-only preserved, **zero event rewrites**. Untraceable pre-v9 commit history is **skipped** (not failed) for this migration only.
- **Migration (2026-08-22, sanctioned — v10 task-local return).** `docs/records/` removed: `goal.md` → `01-goal/artifacts/goal.md`, `conformance-report.md` → `05-engine/08-system-design-validation/artifacts/conformance-report.md`, `s2-record.md` → `06-engine-build/05-s2-envision-grilling/artifacts/s2-record.md` — all REAL files at the exact paths their `artifact-locked` events already record (blob shas unchanged; no event changes).
- **Migration (2026-08-22, sanctioned — v12 the `.ann/` layout).** All ann-owned dirs moved under `<project>/.ann/`: `journey/ → .ann/journey/`, `docs/ → .ann/docs/`, `rules/ → .ann/rules/`; root-level symlinks keep every recorded `artifact-locked` path and the relative artifact links resolving — append-only preserved, **zero event rewrites**. The recognition marker is now `.ann/` (legacy `journey/` accepted for pre-v12 projects).
- **NO migration for v14.** The filename rule is written down, not applied backwards; the schema and event additions are all OPTIONAL fields and a NEW event kind, so every existing `node.json` and every recorded event stays valid as written. The only engine changes this amendment owns are the two `openQuestions` path corrections (§2) — a write-path and a read-path fix, no data change.
- **Platform caveat (recorded):** the v9 migration introduced filesystem symlinks (first use). They are transparent to `readFileSync`/`existsSync` (verified), but are platform-fragile on checkouts that materialize symlinks as text.
