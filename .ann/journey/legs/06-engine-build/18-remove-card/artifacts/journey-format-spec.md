<!-- specs:locked:699615a 2026-08-22 type=spec -->
...
# Journey Format Spec (v13)
*Artifact of task `06-engine-build/18-remove-card`. Type: spec. Complete superseding version — v12 (locked @ 60ee008) + **the description.md card is REMOVED** (§4/§6/§7/§9): nothing ever read it (write-only since v4; no consumer materialized; the derived commands `detail`/`status`/`results` supersede it) and the `card!` command is gone. Search/discovery is deferred (a future `search` command over contracts; not built now). The `.ann/` project layout: all ann-owned files in a project live under `<project>/.ann/` (`journey/` + `docs/` + `rules/`) — subtle, machine-owned, no hand-edits; root-level `journey`/`docs`/`rules` become compat symlinks so recorded paths + relative artifact links keep resolving; the recognition marker is `.ann/` (legacy `journey/` accepted). Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v11 (locked @ 4b4c8c3). Referrers: flow-control-spec · change-protocol · ann-system-design · architecture · AGENTS.md · implementation slices.*
...
6. **`openQuestions`** — SHOULD list known unknowns; `blocking: true` gates the node (and its leg). Never a silent assumption.
7. **Work type** — SHOULD be determinable from intent (validate / envision / spec / implementation / binding / closure — flow-control §7) so verify semantics are known.
...
## 4. File ownership & write timing (v4 — NEW; v7 amended; v9 amended; v10 amended)

| File | What it holds | When written |
|---|---|---|
| `node.json` | leg root = epic contract; task = task contract | **once, at spawn** — never rewritten |
| `events.jsonl` | leg root = **— (absent — v7)** · task = **task lifecycle** (`created`-as-spawn-record/`activated`/`evidence` (with optional structured `commits`/`refs`)/`artifact-locked`/`completed`/`failed`/`superseded` + gates + closure events) | **appended only**; `artifact-locked` on the **producing node** (L1's root holds design's — grandfathered; tasks hold theirs). **(v7) The leg root carries no process log — all process/status events live on tasks; the leg root is structural (node.json + optional artifacts)** |
| `artifacts/` | node outputs — **a ref (symlink into `docs/`) for SHARED document artifacts (specs, designs, architecture); a REAL file for task-local artifacts (a goal, a report, a grandfathered record)** (§14/§15). **Shared content lives in `docs/`, never in the tree** (§15); task-local content lives in the task's `artifacts/` | written when produced; immutable once recorded |
| `docs/` | the **SHARED artifact content files** — the contract stack only, categorized by type (§15) | written when a shared artifact is produced |

**Timing rules:** a leg spawns only after **all previous-leg tasks are `done`** — the derived leg gate (§12), no `completed` event required · a task spawns only after its parent's artifact exists (artifact gate — satisfied by a locked artifact **or structured commit evidence**, §14) · `artifact-locked` is written the moment the artifact locks · appends happen at each lifecycle transition (activated, evidence, artifact-locked, completed, failed, superseded).
...
- **Read discipline:** agents **never read `events.jsonl` directly** — state is understood through the derived commands. `events.jsonl` is a **machine-parse-only log**; a raw read can surface stale or legacy facts that the derived state ignores by construction. The commands are the interface; the log is not.
...
- No hot-migration of existing trees — **except** the sanctioned one-time migrations (v8 flat shape; v9 artifact relocation; v10 task-local return) which are explicit, reviewed, single-commit operations.
...
## 14. The artifact model (v9 — NEW; v10 amended)

**The shape.** An artifact = **a logical name + a file ref + commit clues + a conclusion**:

- **file ref** — a path into the project. **SHARED document artifacts** (specs, system-designs, architecture — the contract stack) live in `docs/`, categorized by type (§15), realized in the task's `artifacts/` as a **symlink**. **TASK-LOCAL artifacts** (a goal, a report, a grandfathered record) are **real files** in the task's `artifacts/`. **External products** (outside the project) are referenced by a **link** (URL) — no file existence check.
- **commit clues (v10) — STRUCTURED EVENTS, not docs.** A code/implementation task's output is its commits + refs, recorded as an `evidence` event with `commits: [{sha, note?}]` and `refs: [paths]` (§3). Machine-readable, never prose-parsed. **The record doc is DEPRECATED** — it was a v9 stopgap (a lockable doc to satisfy the artifact gate); the structured event is the honest form and satisfies F-AC18 directly. Grandfathered: task 05's `s2-record` (its `artifact-locked` event cannot be un-written; the file is task-local).
...
- **Every task concludes.** Completion requires a locked artifact (document) **or** structured commit evidence (F-AC18, §7); an "empty" task records its reason. 
...
## 15. docs/ layout & the migrations (v9 — NEW; v10 amended; v12 amended)

**Where shared content lives.** `docs/` (i.e. `.ann/docs/`) holds **only the SHARED contract stack** — docs cross-referenced by logical name across the journey — categorized by artifact type, one place per file; git tracks changes; the tree holds refs (symlinks) + events. **No catch-all category.** Everything else is task-local and lives in the task's `artifacts/` (real files).

| Category | Contents |
|---|---|
| `docs/specs/` | specs and policies (requirements-spec, format specs, flow-control, functional, resource-registry, change-protocol, depth-policy) |
