...
# Journey Format Spec (v15)
*Artifact of task `06-engine-build/27-journey-format-v15`. Type: spec. Complete superseding version — v14 (locked @ 71fa442) + **the task-splitting + task-naming convention** (never designed before; task ids accreted organically): **§1** — the task id grammar `<NN>-<worktype>-<slug>` for NEW nodes (forward-only: existing ids keep their shapes); **§8** — name discipline amended: the segment cap RAISED 24 → 40 (the grammar is infeasible at 24 — `27-implementation-journey-format-v15` is 35 chars), `-v<N>` permitted in a slug ONLY as the TARGET artifact version, sibling-correction restated; **§13** — task shape amended: one cohesive deliverable per task; **§16 — NEW** — the full task-splitting + task-naming convention: the grammar + worktype→task-kind mapping, splitting rules (one deliverable per task · amendments are always their own task · rework = superseding sibling, never an edit), the task-in-leg vs NEW-LEG decision rule, and slug semantics (short self-describing kebab noun, no dates/abbreviations); the `s<N>` slice tag is DROPPED (historical, project-local to `06-engine-build`). Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v14 (locked @ 71fa442), `core-design` (locked @ f7fb400). Referrers: flow-control-spec · change-protocol · ann-system-design · architecture · AGENTS.md · implementation slices.*
...
- **NO migration for v15.** The task grammar, the cap raise (24 → 40), and the splitting rules are written down, not applied backwards — existing task ids keep their shapes (forward-only, §1/§16). The engine changes this amendment owns are the cap constant in `spawn` (`src/commands/index.ts`) and the `name-discipline` check rule (`src/flow/validators/rules.ts`) + its derived registry — no data change.
