<!-- specs:locked:51c7764 2026-08-22 type=spec -->
...
# Journey Format Spec (v10)
*Artifact of task `06-engine-build/14-format-amendment-v10`. Type: spec. Complete superseding version — v9 (locked @ 3c04f99) + commit clues become STRUCTURED events (evidence gains `commits[]` + `refs[]` — machine-truth, never prose-parsed); the implementation record doc is deprecated (task 05's `s2-record` grandfathered, task-local); `docs/` holds only the shared contract stack (specs / system-designs / architecture — the records/ catch-all removed); task-local outputs (goal, conformance-report) return to their tasks; F-AC18 accepts commit evidence. Produced per the change protocol: amendment node → complete artifact → superseded event. Upstream: `journey/legs/01-goal/artifacts/design.md` (v3, locked), `02-grilling/01-spec-rework/artifacts/requirements-spec.md` (locked), `journey-format-spec` v9 (locked @ 3c04f99). Referrers: flow-control-spec · change-protocol · ann-system-design · architecture · AGENTS.md · implementation slices.*
