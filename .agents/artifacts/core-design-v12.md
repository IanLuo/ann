# Ann Core Design — v12 (draft, for review)

*Re-implementation target. The locked substrate (journey-format v13, store single-writer, packet spec, provider adapter, S4 validators, honesty-layer engines) stays. Incorporated: design reviews #1–#11 (2026-08-25) + owner decisions (2026-08-26: general config; conditional chains; verify fail cycles). The design target: a system that is SIMPLE and FLEXIBLE — the machinery is small and stable; everything that varies (flows, steps, chains, gate sources, rework) is ADJUSTABLE DATA, so later flow changes are sound changes, not design changes.*
...
| chain validity · gate-source routing · **intent idempotence + the transcript-presence discriminator** · **cross-task write permission** · **artifact placement/filename/recorded path** · **frame-write idempotence** · **working-file lifecycle** (task-local, unpoliced; `check()` is event-anchored — a file without an event is outside the integrity model; never pruned — stated, review-8 P4/P12) | **L2 — flow** | SPLIT: STATICALLY VALIDATED (chain validity · gate-source routing · **`when` conditions** — via the step's declared `roles[]`/`produces?[]`/`decisions[]` + the config) · RUNTIME/CONVENTION-ONLY (intent ordering — satisfied BY CONSTRUCTION via deferral, not statically checkable · idempotence · cross-task permission — `supersede!` accepts an arbitrary target, no enforcer exists · placement/filename · frame-write idempotence) — nothing over-claimed (review-8 Q5, review-9 edit 7) |
| **chain linearity** | **configurable** (v12) | chains are LINEAR by default (`flow.conditionals: false`); setting `true` enables `when?` conditions on chain entries — a conditional chain is linear-with-optional-skips (a false condition SKIPS the step: no trace, no events, no effects); condition references are statically validated. The GATE reject bound and gate set/positions are NOT affected — they stay locked (§1:54) |
...
chain entry = { id,
                ...
                when?: {inputMissing?: name | hasOutput?: stepId | verdict?: decision}
                       — CONDITIONAL EXECUTION (v12, enabled by flow.conditionals:
                       true): a false condition SKIPS the step (no trace, no events,
                       no effects — a skip is the absence of execution); references
                       statically validated against the packet + prior }
...
precedence: user config (~/.ann/config.json) > project config > builtin defaults
```
...
- **Chains are LINEAR by default** (v12: `flow.conditionals: false` — the linear-unconditional chain is the default shape, and the escape is a new work type or a step's internal decision). **Setting `flow.conditionals: true` in the general config enables `when?` on chain entries** — linear-with-optional-skips (a false condition skips the step); a runtime branch is then DATA, not code (owner decision 2026-08-26 — this answers review-10's "the change most likely wanted next"). The GATE reject bound and gate set/positions are untouched by the flag.
...
8. **Amendments recorded BEFORE implementation** (each: complete superseding artifact, same logical name, preserving section anchors, back-reference, `superseded` event, referrer re-pointing):
   ...
   - **resource-registry v2 → v3**: `schema` category (:36) + `vocab` instance row (:51-57) + **`config` category (:36 — the general config registry, owner decision 2026-08-26)** + **the :63 code-literal-duplicates reconciliation** (eventTypes/statuses enforcement) — RECONCILES architecture-v2:84; **FOUR LISTED data migrations: vocab.json (artifactTypes → category + versioned) · rules/decide/rules.json (enabled flags) · rules/flow/default.json (chains: workType → string[] becomes {workType → entry[]} with at/inputs/params/verdict/when, + the loader assertion at flow.ts:64 + the validate → idea-validate rename — review-8 P7) · rules/config/default.json (NEW — the general config registry)**
